...
public class NodeAliasLuceneModule extends DAGModule<Collection<DAGNode>> implements
	...
	public Collection<AliasedObject<Character, DAGNode>> findAliasedNodes(
		...
		return (Collection<AliasedObject<Character, DAGNode>>) findNodeInternal(
				alias, caseSensitive, exactString, true);
	}
					...
					if (n != null) {
						...
						if (keep) {
							if (aliased) {
								...
								Character[] charObjectArray = ArrayUtils
										.toObject(charArray);
								nodes.add(new AliasedObject<Character, DAGNode>(
										n, charObjectArray));
							} else
								nodes.add(n);
						}
